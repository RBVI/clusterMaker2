...
import edu.ucsf.rbvi.clusterMaker2.internal.api.CyMatrixFactory;
...
public class CyMatrixFactoryImpl implements CyMatrixFactory {
	...
	public CyMatrix makeLargeMatrix(CyNetwork network, String[] attributes,
		...
		return makeMatrix(network, attributes, selectedOnly, ignoreMissing, transpose, assymetric, MatrixTypes.LARGE);
	}
	...
	public CyMatrix makeSmallMatrix(CyNetwork network, String[] attributes,
	                                boolean selectedOnly, boolean ignoreMissing,
																	boolean transpose, boolean assymetric) {
		return makeMatrix(network, attributes, selectedOnly, ignoreMissing, transpose, assymetric, MatrixTypes.SIMPLE);
	}


	private CyMatrix makeMatrix(CyNetwork network, String[] attributes,
	                            boolean selectedOnly, boolean ignoreMissing,
	...
	private CyMatrix makeAttributeMatrix(CyNetwork network, CyMatrix matrix, List<CyNode> nodeList,
	                                     Map<CyNode,Map<String,Double>> nodeCondMap, String attributeArray[], boolean transpose) {
		if (transpose) {
			int column = 0;
	...
	CyMatrix makeSymmetricalMatrix(CyNetwork network, CyMatrix matrix, List<CyNode> nodeList, 
	...
	CyMatrix makeAssymmetricalMatrix(CyNetwork network, CyMatrix matrix, List<CyNode> sourceNodeList, 
	...
	private CyMatrix makeTypedMatrix(CyNetwork network, int rows, int columns, boolean transpose, MatrixTypes type) {
		int nrows = rows;
		...
		switch (type) {
			case SIMPLE:
				return new CySimpleMatrix(network, nrows, ncolumns);

			case LARGE:
				return new CyColtMatrix(network, nrows, ncolumns);
		}
		return null;
	}

	private Map<CyNode, Map<String,Double>> getNodeCondMap(CyNetwork network, List<CyNode> nodeList, 
	...
	List<CyEdge> getEdgeList(CyNetwork network, boolean selectedOnly) {
	...
	List<CyNode> getNodesFromEdges(CyNetwork network, List<CyEdge> edgeList, String attribute, 
	...
	List<CyNode> getNodesFromEdges(CyNetwork network, List<CyEdge> edgeList, List<CyNode> targetNodeList,
