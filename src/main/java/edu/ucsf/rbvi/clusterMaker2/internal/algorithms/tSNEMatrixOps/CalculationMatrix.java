...
public class CalculationMatrix {
	...
	public static Matrix exp(Matrix m1) {
		Matrix matrix=m1.like(m1.nRows(),m1.nColumns());
	...
	public static boolean [][] biggerThan(Matrix matrix, double value) {
		...
		IntStream.range(0, matrix.nRows()).parallel()
			.forEach(row -> IntStream.range(0, matrix.nColumns())
				.forEach(col -> {
					equals[row][col] = Double.compare(matrix.getValue(row, col), value) == 1;
			}));
		return equals;
	}
	...
	public static Matrix mean(Matrix matrix, int axis) {
		...
		double zerovalue=0;
		if( axis == 0) {
			result = matrix.like(1, matrix.nColumns());
			IntStream.range(0, matrix.nColumns()).parallel()
						.forEach(col -> {
							result.setValue(0, col, matrix.ops().columnMean(col));
						});
		}   else if (axis == 1) {
			result = matrix.like(matrix.nRows(),1);
			IntStream.range(0, matrix.nRows()).parallel()
				.forEach(row -> {
					result.setValue(row, 0, matrix.ops().rowMean(row));
				});
		}   else if (axis == 2) {
			result=matrix.like(1,1);
			double sum = result.ops().sum();
			result.setValue(0, 0, sum/(matrix.nColumns()*matrix.nRows()));
		}else {
	...
	public static Matrix sum(Matrix matrix, int axis) {
		...
		if( axis == 0) {
			...
			IntStream.range(0, matrix.nColumns()).parallel()
						.forEach(col -> {
							result.setValue(0, col, matrix.ops().columnSum(col));
						});
		}   else if (axis == 1) {
			result=matrix.like(matrix.nRows(), 1);
			IntStream.range(0, matrix.nRows()).parallel()
						.forEach(row -> {
							result.setValue(row, 0, matrix.ops().rowSum(row));
						});
		}   else {
	...
	public static Matrix maximum(Matrix matrix, double maxval) {
		...
		Matrix maxed=matrix.like(matrix.nRows(),matrix.nColumns());
		for (int i = 0; i < matrix.nRows(); i++) {
			for (int j = 0; j < matrix.nColumns(); j++) {
	...
	public static Matrix replaceNaN(Matrix matrix, double repl) {
		...
		Matrix result=matrix.like(matrix.nRows(),matrix.nColumns());
		for (int i = 0; i < matrix.nRows(); i++) {
			for (int j = 0; j < matrix.nColumns(); j++) {
	...
	public static Matrix addColumnVector(Matrix matrix, Matrix colvector) {
		...
		IntStream.range(0, matrix.nRows()).parallel()
			.forEach(row -> IntStream.range(0, matrix.nColumns())
				.forEach(col -> {
					result.setValue(row, col, (matrix.getValue(row, col)+colvector.getValue(row, 0)));
			}));
		return result;
	}
	...
	public Matrix parScalarMultiply(Matrix m1,Matrix m2) {
		int ll = 600;
		Matrix result=m1.like(m1.nRows(),m1.nColumns());
	...
	public static Matrix scalarPlus(Matrix m1, double m2) {
		Matrix matrix=m1.copy();
		matrix.ops().addScalar(m2);
		return matrix;
	}
	...
	public static Matrix scalarDivide(Matrix numerator, double denom) {
		Matrix matrix = numerator.copy();
		matrix.ops().divideScalar(denom);
		return matrix;
	}

	public static Matrix scalarDivide(Matrix numerator, Matrix denom) {
		Matrix matrix=numerator.like(numerator.nRows(),numerator.nColumns());
