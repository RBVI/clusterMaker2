...
public class CopyOfGreedyClusterer {
	...
	public CopyOfGreedyClusterer(ConnectedComponent cc){
	...
	public CopyOfGreedyClusterer(ConnectedComponent cc, int limitK) {
		bestCosts = Double.MAX_VALUE;
		this.cc = cc;
		this.listOfElementsSortedByCosts = new int[cc.getNodeNumber()];
		this.icce = cc.getCCEdges();
	...
	private void cluster2(int k2) {
		...
		for (int i = 0; i < this.listOfElementsSortedByCosts.length; i++) {
			...
			ao.affinity = icce.getEdgeCost(i, startNode);
			affinities.put(ao,i);
		}
		...
		HashSet<AffinityObject> affinitiesSet = new HashSet<AffinityObject>();
		affinitiesSet.addAll(affinities.keySet());
		HashSet<AffinityObject> internalAffinitiesSet = new HashSet<AffinityObject>();
		internalAffinitiesSet.addAll(internalAffinities.keySet());
		buildCluster2(affinities,internalAffinities,already,affinitiesSet,internalAffinitiesSet);
		...
		for (int i = 0; i < already.length; i++) {
			...
			ArrayList<Integer> bestcluster = new ArrayList<Integer>();
...
//		return this.cc.getClusteringScore();
		...
		for (int i = 0; i < this.listOfElementsSortedByCosts.length; i++) {
			...
			if(!already[startNode]){
				...
				for (int j = 0; j < this.listOfElementsSortedByCosts.length; j++) {
					if(already[j]) continue;
					AffinityObject ao = new AffinityObject(j);
					ao.affinity = icce.getEdgeCost(j, startNode);
					affinities.put(ao,j);
				}
				startObject = new AffinityObject(startNode);
				startObject.affinity = 0;
	...
	private void buildCluster2(TreeMap<AffinityObject, Integer> affinities, TreeMap<AffinityObject, Integer> internalAffinities,
			boolean[] already, HashSet<AffinityObject> affinitiesSet, HashSet<AffinityObject> internalAffinitiesSet) {
		...
		if(!affinities.isEmpty()){
			while(affinities.lastKey().affinity>0){
				...
				already[id] = true;
				count++;
				for (AffinityObject key : affinitiesSet) {
					key.affinity +=icce.getEdgeCost(key.id, id);
					affinities.put(key,key.id);
				}
				for (AffinityObject key : internalAffinitiesSet) {
					key.affinity +=icce.getEdgeCost(key.id, id);
					affinities.put(key,key.id);
				}
...
//				}
	...
	private void buildCluster(boolean[] already, int startNode, ArrayList<ArrayList<Integer>> clusters){	
		ArrayList<Integer> cluster = new ArrayList<Integer>();
	...
	private double calculateCostsForRemovingNode(ArrayList<Integer> cluster, int j) {
		double costs = 0;
		for (int i = 0; i < cluster.size(); i++) {
	...
	private double calculateCostsForAddingNode(ArrayList<Integer> clusters, int j) {
